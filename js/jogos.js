...
async function carregarJogos() {
  ...
  cont.innerHTML = data.map(j => `
    <div class="card" style="margin-bottom:0;cursor:pointer" onclick="abrirJogo('${j.id}')">
    ...
    cont.innerHTML = `
      <div class="card">
        ...
        <table><thead><tr><th>Min.</th><th>Marcador</th><th>Assistente</th><th>Parte do corpo</th><th>Zona</th><th>Momento</th><th></th></tr></thead><tbody>
          ${(golos || []).map(g => `<tr><td>${g.minuto ?? "—"}'</td><td>${g.marcador?.nome_curto ?? "—"}</td><td>${g.assistente?.nome_curto ?? "—"}</td>
    ...
    cont.innerHTML = `
      <div class="card">
        ...
        <table><thead><tr><th>Atleta</th><th>Passes certos</th><th>Remates à baliza</th><th>Recuperações</th><th></th></tr></thead><tbody>
          ${atletasCache.map(a => { const s = mapaStats[a.id] || {}; return `<tr>
...
async function toggleTitular(atletaId, parte, valor) {
  if (valor) {
    await supabaseClient.from("jogo_titulares").insert({ jogo_id: jogoAtual.id, atleta_id: atletaId, parte });
  } else {
...
async function definirCapitao(atletaId) {
  await supabaseClient.from("jogos").update({ capitao_id: atletaId }).eq("id", jogoAtual.id);
  jogoAtual.capitao_id = atletaId;
}
async function definirVice(atletaId) {
  await supabaseClient.from("jogos").update({ vice_capitao_id: atletaId }).eq("id", jogoAtual.id);
  jogoAtual.vice_capitao_id = atletaId;
}

// ---------- Substituições ----------
async function novaSubstituicao() {
  const parte = prompt("Parte (1 ou 2):"); if (!parte) return;
  const minuto = prompt("Minuto:"); if (minuto === null) return;
  const nomesEntra = atletasCache.map(a => a.nome_curto).join(", ");
  const entraNome = prompt(`Quem entra? (${nomesEntra})`); if (!entraNome) return;
  const saiNome = prompt(`Quem sai? (${nomesEntra})`); if (!saiNome) return;
  const entra = atletasCache.find(a => a.nome_curto.toLowerCase() === entraNome.trim().toLowerCase());
  const sai = atletasCache.find(a => a.nome_curto.toLowerCase() === saiNome.trim().toLowerCase());
  if (!entra || !sai) { alert("Nome não encontrado."); return; }
  await supabaseClient.from("jogo_substituicoes").insert({ jogo_id: jogoAtual.id, parte: Number(parte), minuto: Number(minuto), atleta_entra_id: entra.id, atleta_sai_id: sai.id });
  renderJogoTab("substituicoes");
}
async function eliminarSubstituicao(id) {
  if (!confirm("Eliminar substituição?")) return;
...
async function novoGolo() {
  const nomesLista = atletasCache.map(a => a.nome_curto).join(", ");
  const marcadorNome = prompt(`Quem marcou? (${nomesLista})`); if (!marcadorNome) return;
  const marcador = atletasCache.find(a => a.nome_curto.toLowerCase() === marcadorNome.trim().toLowerCase());
  if (!marcador) { alert("Atleta não encontrada."); return; }
  const assistenteNome = prompt(`Quem assistiu? (deixa vazio se não houve)`);
  const assistente = assistenteNome ? atletasCache.find(a => a.nome_curto.toLowerCase() === assistenteNome.trim().toLowerCase()) : null;
  const parte_corpo = prompt(`Parte do corpo (${PARTES_CORPO.join(" / ")})`);
  const zona = prompt(`Zona (${ZONAS_GOLO.join(" / ")})`);
  const momento = prompt(`Momento do jogo (${MOMENTOS_GOLO.join(" / ")})`);
  const minuto = prompt("Minuto:");
  const parte = prompt("Parte (1 ou 2):");
  await supabaseClient.from("jogo_golos").insert({
    jogo_id: jogoAtual.id, marcador_id: marcador.id, assistente_id: assistente?.id || null,
    parte_corpo: parte_corpo || null, zona: zona || null, momento: momento || null,
    minuto: minuto ? Number(minuto) : null, parte: parte ? Number(parte) : null,
  });
  renderJogoTab("golos");
}
async function eliminarGolo(id) {
  if (!confirm("Eliminar golo?")) return;
...
async function abrirModalEstatisticas(atletaId, nome) {
  ...
  document.getElementById("formEstatisticas").onsubmit = async (e) => {
    ...
    showToast("Estatísticas guardadas.");
...
async function renderResumoJogo(cont) {
  const [{ data: stats }, { data: golos }, { data: titulares }, { data: substituicoes }] = await Promise.all([
  ...
  const somaCampos = {};
  CAMPOS_ESTATISTICA.forEach(([id]) => { somaCampos[id] = (stats || []).reduce((s, r) => s + (r[id] || 0), 0); });

  const linhasClassificacao = atletasCache.map(a => {
    const s = (stats || []).find(x => x.atleta_id === a.id);
    const golosMarcados = (golos || []).filter(g => g.marcador_id === a.id).length;
...
async function gerarPdfJogo() {
  ...
  y = tabelaPdf(doc, y, ["Min.", "Marcador", "Assistente", "Momento"], (golos || []).map(g => [g.minuto ?? "—", g.marcador?.nome_curto ?? "—", g.assistente?.nome_curto ?? "—", g.momento ?? "—"]));

  doc.setFontSize(12); doc.text("Estatísticas individuais e classificação", 14, y); y += 4;
  const linhas = (stats || []).map(s => {
    const golosMarcados = (golos || []).filter(g => g.marcador_id === s.atleta_id).length;
    const assistencias = (golos || []).filter(g => g.assistente_id === s.atleta_id).length;
